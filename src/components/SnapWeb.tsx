...
export default function SnapWeb(props: SnapWebProps) {
  ...
  props.snapcontrol.onChange = (server: Snapcast.Server) => handleChange(server);
  ...
  function updateMediaSession() {
    ...
    try {
      ...
      mediaSession.setActionHandler('play', properties.canPlay ? () => { props.snapcontrol.control(streamId, 'play') } : null);
      mediaSession.setActionHandler('pause', properties.canPause ? () => { props.snapcontrol.control(streamId, 'pause') } : null);
      mediaSession.setActionHandler('previoustrack', properties.canGoPrevious ? () => { props.snapcontrol.control(streamId, 'previous') } : null);
      mediaSession.setActionHandler('nexttrack', properties.canGoNext ? () => { props.snapcontrol.control(streamId, 'next') } : null);
      try {
        mediaSession.setActionHandler('stop', properties.canControl ? () => { props.snapcontrol.control(streamId, 'stop') } : null);
      } catch (error) {
      ...
      >
        <List>
          <ListItem key="about" disablePadding>
            <ListItemButton onClick={() => { setAboutOpen(true); setSettingsOpen(false); }}>
